...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  private measureName = (label: string): number => {
    ...
    ctx.font = '500 17.5px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Noto Sans Telugu", sans-serif';
  ...
  frameOnPov(animate = false): void {
    ...
    const s = Math.max(0.12, Math.min(vw / (2 * reach + PAD * 2), vh / (v.height + PAD * 2), 1.1) * 1.8);
    this.scale.set(s);
